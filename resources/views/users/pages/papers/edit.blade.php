...
@section('title') แก้ไขผลงาน @parent @endsection
...
<section class="content">
  ...
  <div class="card">
    <div class="card-header bg-secondary">
      <h3 class="card-title">แก้ไขผลงาน</h3>
    ...
    <div class="card-body">
    <form method="POST" action="{{ route('papers.update', $paper->id) }}" role="form" id="containerForm" enctype="multipart/form-data">
        @csrf
        @method('PUT')
            <div class="form-group row">
                <label class="col-sm-2 text-right" for="title">ชื่อเรื่อง <span class="text-danger">*</span></label>
                <div class="col-sm-10">
                    <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" value="{{ $paper->title }}" placeholder="ชื่อเรื่อง" >
                    @error('title')
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-10">
                    <input type="text" class="form-control @error('authors') is-invalid @enderror" id="authors" name="authors" value="{{ $paper->authors }}" placeholder="ผู้วิจัย/คณะ" >
                    @error('authors')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-4">
                    <div class="form-check">
                        <input class="form-check-input @error('present') is-invalid @enderror" type="radio" name="present" id="present1" value="1" @if($paper->present ==1) checked @endif>
                        <label class="form-check-label" for="present1">บทความฉบับเต็ม</label>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input @error('present') is-invalid @enderror" type="radio" name="present" id="present2" value="2" @if($paper->present ==2) checked @endif>
                        <label class="form-check-label" for="present2">บทคัดย่อ</label>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input @error('present') is-invalid @enderror" type="radio" name="present" id="present3" value="3" @if($paper->present==3) checked @endif>
                ...
                <div class="col-sm-5">
                    <select class="form-control @error('topic') is-invalid @enderror" name="topic">
                        ...
                        @foreach ($topics as $topic)
                            <option value="{{ $topic->id }}" @if($topic->id == $paper->topic) selected @endif>{{ $topic->id.') '.$topic->name }}</option>
                        @endforeach
                    </select>
                    @error('topic')
                    <span class="invalid-feedback" role="alert">
            ...
            <div class="form-group row">
                ...
                <input type="hidden" id="user_id" name="user_id" value="{{ Auth::user()->id }}">
                    <button type="submit" name="Submit" class="btn btn-primary">ปรับปรุง</button>
